...
import {
  ...
  Loader2, RefreshCw, Bell, ExternalLink, ChevronDown, ChevronUp,
} from 'lucide-react';
...
function ScoreGauge({ score, grade }: { score: number; grade: string }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div
        className="mt-1 px-3 py-0.5 rounded-full text-white text-sm font-bold"
...
function IssueCard({ issue }: { issue: GstIssue }) {
  ...
  const cfg = {
    CRITICAL: { bg: 'bg-red-50',     border: 'border-red-200',   badge: 'bg-red-600 text-white',   icon: <ShieldAlert className="w-4 h-4 text-red-600" /> },
    HIGH:     { bg: 'bg-orange-50',  border: 'border-orange-200', badge: 'bg-orange-500 text-white', icon: <AlertTriangle className="w-4 h-4 text-orange-500" /> },
    MEDIUM:   { bg: 'bg-yellow-50',  border: 'border-yellow-200', badge: 'bg-yellow-500 text-white', icon: <Info className="w-4 h-4 text-yellow-600" /> },
    LOW:      { bg: 'bg-blue-50',    border: 'border-blue-200',   badge: 'bg-blue-500 text-white',   icon: <Info className="w-4 h-4 text-blue-500" /> },
  }[issue.severity];
...
export default function GstHealthPage() {
  ...
  const { data, isLoading, error } = useQuery<GstHealthReport>({
    queryKey:  ['gst-health'],
    queryFn:   async () => { const { data } = await api.get('/reports/gst-health'); return data; },
    staleTime: 5 * 60 * 1000,
  });

  const notifyMutation = useMutation({
    mutationFn: () => api.post('/reports/gst-health/notify'),
    onSuccess:  (res) => {
      qc.setQueryData(['gst-health'], res.data);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <Header
        title="GST Health"
        icon={<ShieldCheck className="w-5 h-5 text-[#1B4F8A]" />}
      />
      ...
      <main className="flex-1 p-4 md:p-6 space-y-6 max-w-4xl mx-auto w-full">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => qc.invalidateQueries({ queryKey: ['gst-health'] })}
              disabled={isLoading}
        ...
        {error && (
          <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
            Failed to load GST health data. Make sure you have the right permissions.
          </div>
        )}
          ...
          <>
            ...
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
              <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: 'Critical', count: report.summary.criticalCount, color: 'text-red-600',    bg: 'bg-red-50 border-red-200' },
                    { label: 'High',     count: report.summary.highCount,     color: 'text-orange-600', bg: 'bg-orange-50 border-orange-200' },
                    { label: 'Medium',   count: report.summary.mediumCount,   color: 'text-yellow-600', bg: 'bg-yellow-50 border-yellow-200' },
                    { label: 'Low',      count: report.summary.lowCount,      color: 'text-blue-600',   bg: 'bg-blue-50 border-blue-200' },
                  ].map(s => (
              ...
              <div className="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {report.isFilingReady
                    ...
                    : <><ShieldAlert className="w-4 h-4 text-red-600" /><span className="text-sm text-red-700 font-medium">{report.summary.criticalCount} critical issue{report.summary.criticalCount > 1 ? 's' : ''} must be fixed before filing</span></>
                  }
                </div>
                {report.checkedAt && (
                  <span className="text-xs text-gray-400">Last checked: {fmt(report.checkedAt)}</span>
                )}
              </div>
            </div>
            ...
            {report.totalIssues === 0 && (
              <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
                ...
                <p className="text-sm text-green-600">No GST compliance issues found. Your books are clean for the next filing.</p>
              </div>
            )}
            ...
            {report.totalIssues > 0 && (
              <div>
                <IssueSection label="CRITICAL"  color="bg-red-600"    issues={report.critical} defaultOpen={true} />
                <IssueSection label="HIGH"       color="bg-orange-500" issues={report.high}     defaultOpen={true} />
                <IssueSection label="MEDIUM"     color="bg-yellow-500" issues={report.medium}   defaultOpen={false} />
                <IssueSection label="LOW"        color="bg-blue-500"   issues={report.low}      defaultOpen={false} />
              </div>
            )}
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
              ...
              <ul className="text-xs text-gray-500 space-y-1 list-disc list-inside">
                ...
                <li>Products sold in last 30 days with missing or placeholder HSN codes</li>
                <li>Duplicate invoice numbers in sales</li>
                <li>GRNs older than 180 days with unpaid balances (ITC reversal risk under Section 16(2)(b))</li>
                <li>Approved GRNs with no supplier GSTIN on record</li>
                <li>Suppliers with invalid GSTIN format</li>
                <li>B2B bills missing customer GSTIN (causes B2B → B2C reporting error)</li>
                <li>Bill cancellation rate in last 30 days (&gt;5% flagged)</li>
