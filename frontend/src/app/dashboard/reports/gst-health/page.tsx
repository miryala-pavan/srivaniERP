...
import {
  ...
  Loader2, RefreshCw, Bell, ExternalLink, ChevronDown, ChevronUp, Calendar, Settings,
} from 'lucide-react';
...
export default function GstHealthPage() {
  ...
  const [fromDateIsDefault, setFromDateIsDefault] = useState(true);

  // Load stored go-live date from GST settings and use it as default
  useEffect(() => {
    api.get('/settings/gst').then(({ data }) => {
      if (data.from_date) {
        setFromDate(data.from_date);
        setFromDateIsDefault(false);
      }
    }).catch(() => {});
  }, []);
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      ...
      <main className="flex-1 p-4 md:p-6 space-y-6 max-w-4xl mx-auto w-full">
        ...
        {/* Date context label */}
        {fromDateIsDefault ? (
          <div className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 -mt-4 flex items-center gap-2">
            <span>Using FY start as default. Set your go-live date in</span>
            <button onClick={() => router.push('/dashboard/settings?tab=gst')} className="font-medium underline hover:no-underline flex items-center gap-0.5">
              Settings &gt; GST <Settings className="w-3 h-3" />
            </button>
            <span>to permanently exclude test data from all GST reports.</span>
          </div>
        ) : (
          <div className="text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2 -mt-4 flex items-center gap-2">
            <span>Showing data from <strong>{fmtDate(fromDate + 'T00:00:00')}</strong> onwards (your go-live date from Settings).</span>
            <button onClick={() => router.push('/dashboard/settings?tab=gst')} className="font-medium underline hover:no-underline flex items-center gap-0.5">
              Change <Settings className="w-3 h-3" />
            </button>
          </div>
        )}
