...
let ReportsService = class ReportsService {
    ...
    async getDailySales(businessId, query) {
        const end = query.endDate ? new Date(query.endDate) : new Date();
        const start = query.startDate ? new Date(query.startDate) : new Date(end.getTime() - 29 * 86400000);
        const conds = [
            client_1.Prisma.sql `sb."businessId" = ${businessId}`,
            client_1.Prisma.sql `sb.status = 'FINAL'`,
            client_1.Prisma.sql `sb."billDate" >= ${dayStart(start)}`,
            client_1.Prisma.sql `sb."billDate" <= ${dayEnd(end)}`,
        ];
        if (query.branchId)
            conds.push(client_1.Prisma.sql `sb."branchId" = ${query.branchId}`);
      ...
      FROM sales_bill sb
      WHERE ${client_1.Prisma.join(conds, ' AND ')}
    ...
    async getStockSummary(businessId, query) {
        const conds = [
            client_1.Prisma.sql `p."businessId" = ${businessId}`,
            client_1.Prisma.sql `p."isActive" = true`,
        ];
        if (query.categoryId)
            conds.push(client_1.Prisma.sql `p."categoryId" = ${query.categoryId}`);
        if (query.search) {
            const like = `%${query.search}%`;
            conds.push(client_1.Prisma.sql `(p.name ILIKE ${like} OR p.barcode ILIKE ${like})`);
        }
        const branchFilter = query.branchId
            ? client_1.Prisma.sql `AND sl."branchId" = ${query.branchId}`
            : client_1.Prisma.sql ``;
        const rows = await this.prisma.$queryRaw(client_1.Prisma.sql `
      SELECT
        p.id,
    ...
    async getLowStock(businessId, branchId) {
        const branchFilter = branchId
            ? client_1.Prisma.sql `AND sl."branchId" = ${branchId}`
            : client_1.Prisma.sql ``;
        const rows = await this.prisma.$queryRaw(client_1.Prisma.sql `
      SELECT
        p.id,
    ...
    async getProfitReport(businessId, query) {
        const end = query.endDate ? new Date(query.endDate) : new Date();
    ...
    async getCashSummary(businessId, query) {
        const targetDate = query.date ? new Date(query.date) : new Date();
        const conds = [
            client_1.Prisma.sql `pc."businessId" = ${businessId}`,
            client_1.Prisma.sql `ps."shiftDate" >= ${dayStart(targetDate)}`,
            client_1.Prisma.sql `ps."shiftDate" <= ${dayEnd(targetDate)}`,
        ];
        if (query.branchId)
            conds.push(client_1.Prisma.sql `ps."branchId" = ${query.branchId}`);
        const rows = await this.prisma.$queryRaw(client_1.Prisma.sql `
      SELECT
        ps.id                    AS shift_id,
    ...
    async getDashboard(businessId) {
        ...
        const [todayAgg, yesterdayAgg, cashMismatchRows, lowStockRows, pendingGRNs, pendingPaymentSuppliers, topProductRows,] = await Promise.all([
