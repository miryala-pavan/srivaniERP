...
function validateHsn(hsn) {
    if (!/^[0-9]+$/.test(hsn) || ![2, 4, 6, 8].includes(hsn.length)) {
        throw new common_1.BadRequestException(`HSN code "${hsn}" is invalid. Must be 2, 4, 6, or 8 numeric digits per GST rules.`);
    }
}
function calculateItemGst(unitPrice, quantity, discountPercent, gstRate, isIntraState) {
...
let PosService = class PosService {
    ...
    async createCounter(businessId, dto) {
        ...
        if (existing)
            throw new common_1.ConflictException(`Counter code "${dto.code}" already exists`);
    ...
    async openShift(businessId, userId, dto) {
        ...
        if (counter.status !== 'ACTIVE') {
            throw new common_1.BadRequestException('Counter is not active');
        }
    ...
    async getTodayShifts(businessId, dateStr) {
    ...
    async forceCloseShift(businessId, shiftId, managerName) {
        const shift = await this.prisma.posShift.findFirst({
    ...
    async createBill(businessId, userId, dto) {
        ...
        if (!shift)
            throw new common_1.NotFoundException('Shift not found or already closed');
        ...
        if (!dayRecord) {
            throw new common_1.BadRequestException('Day has not been opened yet. A manager must open the day before billing.');
        }
        if (dayRecord.status === 'COMPLETED') {
            throw new common_1.BadRequestException("Today's day is closed. Ask a manager to open a new day before billing.");
        }
        ...
        if (!cashierActiveShift) {
            throw new common_1.BadRequestException('No active shift. Start a shift before billing.');
        }
        ...
        if (!billSeries)
            throw new common_1.BadRequestException('No active bill series. Complete business setup first.');
        const businessState = business.stateCode;
        ...
        for (const item of dto.items) {
            ...
            if (!tax)
                throw new common_1.NotFoundException(`Tax ${item.taxId} not found`);
            validateHsn(product.hsnCode);
            const gstRate = Number(tax.taxRate);
            if (!VALID_GST_RATES.has(gstRate)) {
                throw new common_1.BadRequestException(`GST rate ${gstRate}% for "${product.name}" is not a valid Indian GST rate. ` +
        ...
        if (dto.paymentMode === create_bill_dto_1.PaymentModeEnum.SPLIT) {
            const splitTotal = r2((dto.cashAmount ?? 0) + (dto.upiAmount ?? 0) + (dto.cardAmount ?? 0));
            if (Math.abs(splitTotal - grandTotal) > 0.01) {
                throw new common_1.BadRequestException(`Split amounts (₹${splitTotal}) must equal grand total (₹${grandTotal})`);
            }
            paidAmount = grandTotal;
        }
        if (paidAmount > grandTotal) {
            throw new common_1.BadRequestException('Paid amount cannot exceed grand total');
        }
        ...
        const bill = await this.prisma.$transaction(async (tx) => {
            ...
            const bill = await tx.salesBill.create({
                data: {
                    ...
                    cashAmount: dto.paymentMode === create_bill_dto_1.PaymentModeEnum.CASH ? r2(dto.cashAmount ?? grandTotal) :
                        dto.paymentMode === create_bill_dto_1.PaymentModeEnum.SPLIT ? r2(dto.cashAmount ?? 0) : null,
                    upiAmount: dto.paymentMode === create_bill_dto_1.PaymentModeEnum.UPI ? grandTotal :
                        dto.paymentMode === create_bill_dto_1.PaymentModeEnum.SPLIT ? r2(dto.upiAmount ?? 0) : null,
                    cardAmount: dto.paymentMode === create_bill_dto_1.PaymentModeEnum.CARD ? grandTotal :
                        dto.paymentMode === create_bill_dto_1.PaymentModeEnum.SPLIT ? r2(dto.cardAmount ?? 0) : null,
                },
            });
            for (let idx = 0; idx < calcItems.length; idx++) {
                ...
                if (!isEstimate) {
                    ...
                    const orderBy = product.expiryTracking
            ...
            if (dto.paymentMode === create_bill_dto_1.PaymentModeEnum.CASH) {
                shiftInc.totalCash = { increment: grandTotal };
            }
            else if (dto.paymentMode === create_bill_dto_1.PaymentModeEnum.UPI) {
                shiftInc.totalUpi = { increment: grandTotal };
            }
            else if (dto.paymentMode === create_bill_dto_1.PaymentModeEnum.CARD) {
                shiftInc.totalCard = { increment: grandTotal };
            }
            else if (dto.paymentMode === create_bill_dto_1.PaymentModeEnum.SPLIT) {
                if (dto.cashAmount)
                    shiftInc.totalCash = { increment: r2(dto.cashAmount) };
                if (dto.upiAmount)
                    shiftInc.totalUpi = { increment: r2(dto.upiAmount) };
                if (dto.cardAmount)
                    shiftInc.totalCard = { increment: r2(dto.cardAmount) };
            }
    ...
    async getEstimates(businessId, page = 1, limit = 20) {
        const skip = (page - 1) * limit;
        const where = { businessId, billType: 'ESTIMATE' };
        const now = new Date();
    ...
    async convertEstimate(businessId, userId, estimateId, targetBillType) {
        ...
        if (!billSeries)
            throw new common_1.BadRequestException('No bill series for ' + targetBillType);
        ...
        return this.prisma.$transaction(async (tx) => {
            for (const item of estimate.items) {
                ...
                if (available < Number(item.quantity)) {
                    ...
                    throw new common_1.BadRequestException(`Insufficient stock for "${p?.name}". Available: ${available}`);
                }
            }
            ...
            for (const item of estimate.items) {
                await tx.salesItem.create({
                    data: { ...item, id: undefined, billId: newBill.id },
                });
                await tx.stockLedger.create({
    ...
    async cancelEstimate(businessId, estimateId) {
        const estimate = await this.prisma.salesBill.findFirst({
            where: { id: estimateId, businessId, billType: 'ESTIMATE', estimateStatus: 'OPEN' },
        });
        if (!estimate)
    ...
    async checkStockAfterSale(businessId, branchId, productIds) {
        ...
        for (const productId of unique) {
            try {
                ...
                const minimumStockLevel = Number(product.minimumStockLevel ?? 0);
    ...
    async getBills(businessId, query) {
        ...
        if (query.startDate || query.endDate) {
            where.billDate = {};
            if (query.startDate)
                where.billDate.gte = new Date(query.startDate);
            if (query.endDate)
                where.billDate.lte = new Date(query.endDate + 'T23:59:59');
        }
        const [data, total] = await this.prisma.$transaction([
            this.prisma.salesBill.findMany({
                where,
    ...
    async getBillById(businessId, id) {
    ...
    async getStock(businessId, productId) {
    ...
    async createHold(userId, userName, businessId, dto) {
        const branchId = await this.getFirstBranchId(businessId);
    ...
    async searchBills(businessId, query) {
        const where = {
            businessId,
    ...
    async getFullBillForPrint(id, businessId) {
        const bill = await this.prisma.salesBill.findFirst({
            where: { id, businessId },
            include: {
                items: {
    ...
    async logDuplicatePrint(id, businessId, userId, userName, counterName) {
    ...
    async voidBill(businessId, userId, userRole, userShiftId, billId, reason, userName) {
        const bill = await this.prisma.salesBill.findFirst({
        ...
        if (bill.status === 'CANCELLED')
            throw new common_1.BadRequestException('Cannot void a cancelled bill');
        if (bill.billType === 'ESTIMATE')
            throw new common_1.BadRequestException('Cannot void an estimate');
        ...
        if (closure)
            throw new common_1.BadRequestException('Cannot void bill: today\'s day closure is already completed');
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        const billDay = new Date(bill.billDate);
        billDay.setHours(0, 0, 0, 0);
        if (billDay.getTime() !== today.getTime()) {
            throw new common_1.BadRequestException('Can only void bills created today');
        }
        const isSupervisor = ['SUPER_ADMIN', 'BRANCH_MANAGER'].includes(userRole);
        if (!isSupervisor) {
            if (bill.createdById !== userId) {
                throw new common_1.BadRequestException('Cashiers can only void their own bills');
            }
            if (userShiftId && bill.shiftId !== userShiftId) {
                throw new common_1.BadRequestException('Cashiers can only void bills from their current shift');
            }
        }
    ...
    async createCreditNote(businessId, userId, userName, dto) {
        ...
        if (!originalBill)
            throw new common_1.NotFoundException('Original bill not found or not finalized');
        if (originalBill.isVoided)
            throw new common_1.BadRequestException('Cannot create credit note for a voided bill');
        const originalItemMap = new Map(originalBill.items.map((i) => [i.productId, i]));
        for (const item of dto.items) {
            const orig = originalItemMap.get(item.productId);
            if (!orig)
                throw new common_1.BadRequestException(`Product ${item.productId} not found in original bill`);
            if (item.quantity > Number(orig.quantity)) {
                throw new common_1.BadRequestException(`Cannot return more than purchased quantity for ${orig.productName}`);
            }
        }
        const fy = await this.prisma.financialYear.findFirst({
            where: { businessId, isActive: true },
    ...
    async getCreditNotes(businessId, query) {
        const page = query.page ?? 1;
        const limit = query.limit ?? 20;
        const skip = (page - 1) * limit;
        const where = { businessId };
        if (query.originalBillId)
            where.originalBillId = query.originalBillId;
        if (query.customerName)
    ...
    async getCreditNoteById(businessId, id) {
    ...
    async createHistoricalBill(businessId, userId, dto) {
        const FY_START = new Date('2026-04-01');
        const today = new Date();
        today.setHours(23, 59, 59, 999);
        const billDate = new Date(dto.billDate);
        if (isNaN(billDate.getTime()))
            throw new common_1.BadRequestException('Invalid bill date');
        if (billDate > today)
            throw new common_1.BadRequestException('Cannot enter future date');
        if (billDate < FY_START)
            throw new common_1.BadRequestException('Cannot enter bills before April 1, 2026');
        const business = await this.prisma.business.findUnique({ where: { id: businessId } });
        if (!business)
            throw new common_1.NotFoundException('Business not found');
        const branchId = await this.getFirstBranchId(businessId);
        ...
        if (!fy)
            throw new common_1.BadRequestException('No financial year found for this date');
        ...
        if (dto.type === 'B2C_SUMMARY') {
            if (dto.taxableAmount === undefined)
                throw new common_1.BadRequestException('taxableAmount required for B2C summary');
        ...
        if (!dto.invoiceNumber)
            throw new common_1.BadRequestException('invoiceNumber required for B2B bill');
        if (!dto.customerName)
            throw new common_1.BadRequestException('customerName required for B2B bill');
        if (!dto.customerGstin)
            throw new common_1.BadRequestException('customerGstin required for B2B bill');
        if (dto.taxableAmount === undefined)
            throw new common_1.BadRequestException('taxableAmount required');
    ...
    async createHistoricalBillsBulk(businessId, userId, bills) {
        const created = [];
        const errors = [];
    ...
    async getHistoricalBills(businessId, query) {
        const page = Math.max(1, query.page ?? 1);
        const limit = Math.min(100, query.limit ?? 50);
        const skip = (page - 1) * limit;
        const where = { businessId, isHistorical: true };
        if (query.type === 'B2C')
            where.isB2B = false;
        if (query.type === 'B2B')
            where.isB2B = true;
    ...
    async deleteHistoricalBill(businessId, id) {
        const bill = await this.prisma.salesBill.findFirst({
            where: { id, businessId, isHistorical: true },
        });
        if (!bill)
            throw new common_1.NotFoundException('Historical bill not found');
        await this.prisma.salesBill.delete({ where: { id } });
        return { message: 'Deleted' };
    }
    async getFirstBranchId(businessId) {
        const branch = await this.prisma.branch.findFirst({
            where: { businessId, isActive: true },
            orderBy: { createdAt: 'asc' },
        });
        if (!branch) {
            throw new common_1.BadRequestException('No active branch found. Complete business setup at POST /api/business/setup');
        }
        return branch.id;
    }
