    ...
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
      ...
      <tr>
        <td style="background:#fafafa;border-top:1px solid #eee;padding:20px 32px;text-align:center;">
          ...
          <p style="margin:0;font-size:11px;color:#aaa;">© Srivani Stores, Sangareddy · This is a transactional email</p>
        </td>
      </tr>
  ...
  const rows = items.map(i => `
    <tr>
      <td style="padding:10px 0;border-bottom:1px solid #f0f0f0;font-size:13px;color:#333;">
        ...
        <span style="color:#888;font-size:11px;">${i.packLabel} × ${i.quantity}</span>
      </td>
...
function totalsBlock(subtotal: number, deliveryFee: number, total: number): string {
  return `<table width="100%" cellpadding="0" cellspacing="0" style="margin-top:4px;">
    <tr>
      ...
      <td style="font-size:13px;color:#333;text-align:right;">₹${subtotal.toFixed(2)}</td>
    </tr>
    <tr>
      <td style="font-size:13px;color:#888;padding:4px 0;">Delivery</td>
      <td style="font-size:13px;color:${deliveryFee === 0 ? BRAND_GREEN : '#333'};text-align:right;font-weight:${deliveryFee === 0 ? '700' : '400'};">${deliveryFee === 0 ? 'FREE' : `₹${deliveryFee.toFixed(2)}`}</td>
    </tr>
    <tr>
      <td style="font-size:16px;font-weight:800;color:#111;padding:10px 0 4px;border-top:2px solid #eee;">Total</td>
      <td style="font-size:16px;font-weight:800;color:#111;text-align:right;padding:10px 0 4px;border-top:2px solid #eee;">₹${total.toFixed(2)}</td>
    </tr>
  </table>`;
}
...
export class EmailService {
  ...
  async sendOrderPlaced(order: {
    ...
    const html = base(`
      ...
      <div style="background:${BRAND_LIGHT};border-radius:8px;padding:14px 18px;font-size:13px;color:#444;margin-bottom:20px;">
        <strong>Payment:</strong> ${isCOD ? 'Cash on Delivery' : 'Online (Razorpay)'} &nbsp;·&nbsp;
    ...
    await this.send(
      order.customerEmail,
      `Order Confirmed — ${order.orderNumber} | Srivani Stores`,
      html,
    );
  }
  ...
  async sendPaymentConfirmed(order: {
    ...
    const html = base(`
      ...
      <p style="margin:0 0 20px;font-size:14px;color:#555;">
        Hi ${order.customerName}, your payment of <strong>₹${order.total.toFixed(2)}</strong> was successful.
  ...
  async sendStatusUpdate(order: {
    ...
    const messages: Record<string, { subject: string; heading: string; body: string; icon: string }> = {
      ...
      CANCELLED: {
        ...
        body: order.deliveryType !== 'STORE_PICKUP'
          ? 'Your order has been cancelled. If you paid online, a refund will be processed in 5–7 working days.'
