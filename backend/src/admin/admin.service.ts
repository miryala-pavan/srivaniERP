...
export class AdminService {
  ...
  async seed(businessId: string) {
    const business = await this.prisma.business.findUnique({ where: { id: businessId } });
    if (!business) return { message: 'Business not found', seeded: false };
    ...
    // ── Products ──────────────────────────────────────────
    // Build sub-category map too (needed for product seeding with precise sub-cats)
    const allCats = await this.prisma.category.findMany({ where: { businessId } });
    const allCatMap: Record<string, string> = {};
    for (const c of allCats) allCatMap[c.code] = c.id;
    ...
    for (const u of demoUserDefs) {
      ...
      if (!existing) {
        ...
        // passwordHash gets a random unusable value so PIN-only users cannot login via password
  ...
  async fixProductData(businessId: string) {
  ...
  async resetBillSeries(businessId: string, dto: {
    ...
    if (!fy) throw new BadRequestException('No active financial year found');
    ...
    for (const [billType, start] of Object.entries(typeMap)) {
      ...
      if (!Number.isInteger(start) || start < 1) {
        throw new BadRequestException(`Starting number for ${billType} must be a positive integer`);
      }
      ...
      if (start - 1 < series.currentNumber) {
        throw new BadRequestException(
          `Cannot set ${billType} starting number to ${start} — ${series.currentNumber} bills already issued this year. Choose a number greater than ${series.currentNumber}.`,
        );
      }
  ...
  async seedDepartments(businessId: string) {
    const DEPT_SEED: Array<{ name: string; code: string; order: number; categories: string[] }> = [
    ...
    for (const d of DEPT_SEED) {
      ...
      for (let ci = 0; ci < d.categories.length; ci++) {
        ...
        if (!exists) {
          ...
          if (subCodeExists) finalSubCode = `${subCode}X`;
  ...
  async migrateOrphansPhase1(businessId: string) {
    return this.prisma.$transaction(async (tx) => {
      const summary = {
        brand:           'already_existed' as string,
        brandId:         '' as string,
        deptsCreated:    [] as string[],
        catsCreated:     [] as string[],
        subCatsCreated:  [] as string[],
        productsMovedLog: [] as Array<{
          productId: string; name: string;
          oldCategoryId: string; oldCategoryName: string;
          newDepartmentId: string; newDepartmentCode: string;
          newCategoryId: string; newCategoryName: string; newParentName: string;
        }>,
      };
      ...
      // ── 4c: 18 new categories ─────────────────────────────────────────────
      const NEW_CAT_DEFS: Array<{ deptCode: string; name: string; codeBase: string; sortOrder: number }> = [
      ...
      const catMap: Record<string, { id: string; code: string; departmentId: string }> = {};
      ...
      // ── 4d: "General" sub-cat under each new category ────────────────────
      for (const [, cat] of Object.entries(catMap)) {
        ...
        if (!exists) {
          ...
          if (subCodeExists) subCode = `${subCode}X`;
      ...
      // ── 4e: Migrate 16 products ───────────────────────────────────────────
      const PRODUCT_MAP: Array<{ oldName: string; deptCode: string; catName: string }> = [
